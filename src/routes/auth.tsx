...
function AuthPage() {
  ...
  const signUp = async () => {
    ...
    if (error) return toast.error(friendlyError(error.message));
    if (data.session) {
      // Only a brand-new account lands here with an immediate session (no
      // email confirmation required for this project) — safe to send
      // straight into onboarding. Sign-in never routes here, so an existing
      // customer's session is never redirected into the wizard.
