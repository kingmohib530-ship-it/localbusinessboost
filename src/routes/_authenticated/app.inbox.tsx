...
function conversationStatus(c: ConversationComputed): LvStatus {
  ...
  return c.ai_paused ? "done" : "automated";
}
...
function Inbox() {
  ...
  async function attemptSend(localId: string, text: string) {
    ...
    setPending((prev) =>
      prev.map((p) => (p.localId === localId ? { ...p, state: "sending" as const } : p)),
    );
    try {
      ...
      const sentAt = (json.message as MessageRow).sent_at;
      ...
      setPending((prev) =>
        prev.map((p) => (p.localId === localId ? { ...p, state: "failed" as const } : p)),
      );
    }
  }
  ...
  function handleSend() {
    ...
    setPending((prev) => [...prev, { localId, message: text, state: "sending" }]);
                ...
                <>
                  ...
                  {pending.map((p) => (
                    <div key={p.localId} className="flex justify-end">
                      ...
                      >
                        <p className="whitespace-pre-wrap break-words">{p.message}</p>
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          >
                            {p.state === "failed" ? "Not delivered" : "Sending..."}
                          </p>
                          {p.state === "failed" && (
                            <button
                              type="button"
                              onClick={() => attemptSend(p.localId, p.message)}
                              className="lv-meta text-primary underline"
                            >
                              Retry
                            </button>
                          )}
            ...
            <div className="shrink-0 border-t border-border p-3 pb-[max(0.75rem,env(safe-area-inset-bottom))]">
              {selectedConversation.channel === "web_chat" && (
                <p className="lv-meta text-muted-foreground mb-2">
                  Web chat replies aren't delivered live yet - this pauses the AI and saves your
                  reply for your own record.
                </p>
              )}
