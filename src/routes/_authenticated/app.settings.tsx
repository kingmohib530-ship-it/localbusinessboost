...
function Settings() {
  ...
  // already-consumed-by-the-cron fields with no prior UI anywhere.
  ...
  async function loadAll() {
    ...
    const { data: profile, error } = await supabase
      .from("profiles")
      .select(
        "business_name, industry, city, timezone, subscription_tier, subscription_status, verification_status, daily_brief_enabled, daily_brief_channel, owner_phone, twilio_verified_at, twilio_phone_number, google_place_id",
      )
      .eq("id", user.id)
      .maybeSingle();
    if (error) {
    ...
    setTwilioConnected(!!profile?.twilio_verified_at);
    setTwilioNumber(profile?.twilio_phone_number || null);
            ...
            <div className="rounded-md border border-border bg-card p-4 md:p-6 space-y-3">
              ...
              <div className="flex items-center justify-between gap-3 rounded-sm border border-border px-3 py-2.5">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <p className="lv-body text-foreground">Phone (Twilio)</p>
                    <p className="lv-meta text-muted-foreground">
                      {twilioConnected ? `Connected — ${twilioNumber}` : "Not connected"}
                    </p>
                  </div>
                </div>
